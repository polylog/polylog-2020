...
jQuery(document).ready(($) => {
  ...
  function fundraisingDetails() {
    ...
    const $fundraisingRequired = $('.form__fieldset.is-fundraising .is-optional-required');

    if ($services.val() === 'Фандрайзинг' && $fieldsetFundraising.is(':hidden')) {
      $fundraisingRequired.prop('required', true);
      $fieldsetFundraising.slideDown(1000);
    } else if ($services.val() !== 'Фандрайзинг' && $fieldsetFundraising.is(':visible')) {
